...
class MySpider(CrawlSpider):
    name = "test-crawler"
    target_domains = ["http://**.**.**.***:9090/"] # list of domains that will be allowed to be crawled
    start_urls = ["http://**.**.**.***:9090/"] # list of starting urls for the crawler
    ...
    def parse_my_url(self, response):
        ...
        if response.status in report_if:  # if the response matches then creates a MyItem
            ...
            yield item
        yield None  # if the response did not m
